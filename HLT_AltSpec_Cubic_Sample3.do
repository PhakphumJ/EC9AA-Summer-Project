*** This is the code for doing the decomposition using the alternative specification *** Implement through the iterative algorithm used for the baseline specification. ** Cubic polynomials; Sample 3.
...
while `diff' > $precision & `iter' < $max_iter{
	...
	* Cohort Profile
	...
	foreach num of numlist 2(1)`n_cohort' { // have to start from 2 since the first is the coeff of base group, which is = 0.
		replace profile_coh = coef_mat[1, 3 + `num'] if _n==`num' // since the coeff of cohort come after the coeff of experience groups.
...
save "Data\Temp\HLT_results_sample3_UnCr_AltSpec_cubic.dta", replace
...
save "Data\Temp\HLT_results_sample3_Cr_AltSpec_cubic.dta", replace