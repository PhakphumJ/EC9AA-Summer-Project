*** This is the code for doing the decomposition using the alternative specification *** Implement through the iterative algorithm used for the baseline specification. ** Quartic polynomials; Sample 2.
...
// Do year - 1968 instead since the inteval between is not constant.
...
while `diff' > $precision & `iter' < $max_iter{
	...
	* Decomposing (the main regression)
	reg Def_logrealwage_temp exp_lin exp_sq exp_cube exp_quar i.coh_rlb dea_*star [pweight=perwt] // (they used aweight, I think pweight is more appropriate) 
	...
	* Experience Profile
	...
	replace exp_quar_eff = coef_mat[1,4]
	
	replace profile_wexp = exp_lin_eff*(profile_wexp_plot - exp_bar) + exp_sq_eff*(profile_wexp_plot - exp_bar)^2 + exp_cb_eff*(profile_wexp_plot - exp_bar)^3 + exp_quar_eff*(profile_wexp_plot - exp_bar)^4
	...
	* Cohort Profile
	...
	foreach num of numlist 2(1)`n_cohort' { // have to start from 2 since the first is the coeff of base group, which is = 0.
		replace profile_coh = coef_mat[1, 4 + `num'] if _n==`num' // since the coeff of cohort come after the coeff of experience groups.
	}
	...
	* Year Profile (Note: this step takes into account that repeated cross-section might be NOT at an yearly frequency)
	...
	foreach num of numlist 1(1)`n_year2' {
		replace profile_year = coef_mat[1, 4 + `n_cohort' + `num'] if _n==`num' +2
...
save "Data\Temp\HLT_results_sample2_UnCr_AltSpec_quartic.dta", replace
...
save "Data\Temp\HLT_results_sample2_Cr_AltSpec_quartic.dta", replace
