...
while `diff' > $precision & `iter' < $max_iter{
	...
	* Cohort Profile
	...
	foreach num of numlist 2(1)`n_cohort' { // have to start from 2 since the first is the coeff of base group, which is = 0.
		replace profile_coh = coef_mat[1, 4 + `num'] if _n==`num' // since the coeff of cohort come after the coeff of experience groups (first four coef).
...
save "Data\Temp\HLT_results_sample1_UnCr_AltSpec_quartic.dta", replace
...
save "Data\Temp\HLT_results_sample1_Cr_AltSpec_quartic.dta", replace