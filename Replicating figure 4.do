...
foreach y of local years_list {
    ...
    // We use `replace` instead of `gen` because the variables are created on the first iteration
    // and then updated for subsequent iterations.
    
    replace income_below_2_5pct = (incwage <= `p2_5') if year == `y'
    

    replace income_above_97_5pct = (incwage >= `p97_5') if year == `y'
}